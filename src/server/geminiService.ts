...
// Initialize Gemini client with user-agent header as required for telemetry
...
// Simple in-memory rate limiter to protect API Key quota
...
// Configurable limits
const MAX_REQUESTS_PER_IP_DAY = 500; // Limit per IP per day
const MAX_REQUESTS_TOTAL_DAY = 2000;  // High but safe overall limit per day
...
function resetLimitsIfNeeded() {
  ...
  // Global reset
  if (now - lastGlobalReset > ONE_DAY_MS) {
    totalDailyRequests = 0;
    lastGlobalReset = now;
    ipLimits.clear();
    console.log("[GeminiService] Global daily rate limits have been reset.");
  }
}

export function checkAndIncrementRateLimit(ip: string): { allowed: boolean; message?: string } {
  resetLimitsIfNeeded();

  // 1. Check overall total daily limit
  if (totalDailyRequests >= MAX_REQUESTS_TOTAL_DAY) {
    return {
      allowed: false,
      message: `Hệ thống đã đạt giới hạn tối đa ${MAX_REQUESTS_TOTAL_DAY} yêu cầu/ngày để bảo vệ API key. Vui lòng thử lại vào ngày mai.`
    };
  }

  // 2. Check IP specific daily limit
  const clientData = ipLimits.get(ip) || { count: 0, lastReset: Date.now() };
  const now = Date.now();
  const ONE_DAY_MS = 24 * 60 * 60 * 1000;

  if (now - clientData.lastReset > ONE_DAY_MS) {
    clientData.count = 0;
    clientData.lastReset = now;
  }

  if (clientData.count >= MAX_REQUESTS_PER_IP_DAY) {
    return {
      allowed: false,
      message: `Địa chỉ IP của bạn đã vượt quá giới hạn an toàn ${MAX_REQUESTS_PER_IP_DAY} yêu cầu/ngày. Hãy thử lại vào ngày mai hoặc liên hệ quản trị viên.`
    };
  }
...
export function getRateLimitStatus(ip: string) {
  ...
  const clientData = ipLimits.get(ip) || { count: 0, lastReset: Date.now() };
  return {
    ipLimit: MAX_REQUESTS_PER_IP_DAY,
    ipUsage: clientData.count,
    ipRemaining: Math.max(0, MAX_REQUESTS_PER_IP_DAY - clientData.count),
    totalLimit: MAX_REQUESTS_TOTAL_DAY,
    totalUsage: totalDailyRequests,
    totalRemaining: Math.max(0, MAX_REQUESTS_TOTAL_DAY - totalDailyRequests),
  };
}

// Helper withRetry with correct official model names (gemini-3.1-pro-preview -> gemini-3.5-flash)
async function withRetry<T>(fn: (model: string) => Promise<T>, maxRetries = 3): Promise<T> {
  let lastError: any;
  let currentModel = "gemini-3.1-pro-preview"; // Primary model for complex math tasks
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (errorStr.includes("429") || errorStr.includes("RESOURCE_EXHAUSTED")) {
        console.warn(`[GeminiService] Model ${currentModel} Quota Exceeded (429). Trying fallback model...`, error);
        if (currentModel === "gemini-3.1-pro-preview") {
          currentModel = "gemini-3.5-flash"; // Valid high-performance flash model fallback
          i--; // retry with fallback without losing retry count
          continue;
        }
        throw new Error("QUOTA_EXCEEDED");
      }

      if (errorStr.includes("Rpc failed") || errorStr.includes("500") || errorStr.includes("503") || errorStr.includes("xhr error") || errorStr.includes("fetch")) {
        console.warn(`[GeminiService] Attempt ${i + 1} failed for ${currentModel}, retrying...`, error);
...
export async function solveAdvancedMathOnServer(
  prompt: string, 
  mode: string, 
...
export async function solveComplexOptimizationOnServer(tab: string, query: string, difficulty: string): Promise<any> {
  const rules = `
...
export async function analyzePolynomialOnServer(coeffs: string): Promise<any> {
  const response = await withRetry((model) => ai.models.generateContent({
    model: model,
